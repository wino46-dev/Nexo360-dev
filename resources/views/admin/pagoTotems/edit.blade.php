...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.pagoTotem.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.pago-totems.update", [$pagoTotem->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('emisor') ? 'is-invalid' : '' }}" name="emisor_id" id="emisor_id" required>
                    @foreach($emisors as $id => $entry)
                        <option value="{{ $id }}" {{ (old('emisor_id') ? old('emisor_id') : $pagoTotem->emisor->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('receptor') ? 'is-invalid' : '' }}" name="receptor_id" id="receptor_id" required>
                    @foreach($receptors as $id => $entry)
                        <option value="{{ $id }}" {{ (old('receptor_id') ? old('receptor_id') : $pagoTotem->receptor->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('sesion') ? 'is-invalid' : '' }}" name="sesion_id" id="sesion_id">
                    @foreach($sesions as $id => $entry)
                        <option value="{{ $id }}" {{ (old('sesion_id') ? old('sesion_id') : $pagoTotem->sesion->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('importe') ? 'is-invalid' : '' }}" type="number" name="importe" id="importe" value="{{ old('importe', $pagoTotem->importe) }}" step="0.01" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('factura') ? 'is-invalid' : '' }}" type="text" name="factura" id="factura" value="{{ old('factura', $pagoTotem->factura) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('tipo_operacion') ? 'is-invalid' : '' }}" name="tipo_operacion" id="tipo_operacion">
                    ...
                    @foreach(App\Models\PagoTotem::TIPO_OPERACION_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('tipo_operacion', $pagoTotem->tipo_operacion) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
