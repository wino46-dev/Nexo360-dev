...
@section('content')
    <div class="card">
        <div class="card-header">
            {{ trans('global.create') }} {{ trans('cruds.sociedad.title_singular') }}
        </div>

        <div class="card-body">
            <form method="POST" action="{{ route('admin.sociedads.store') }}" enctype="multipart/form-data">
                @csrf
                <div class="row">
                ...
                <div class="form-group">
                    ...
                    <div class="needsclick dropzone {{ $errors->has('imagenes_pagina_1') ? 'is-invalid' : '' }}"
                        id="imagenes_pagina_1-dropzone">
                    </div>
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control {{ $errors->has('pago_html_inferior') ? 'is-invalid' : '' }}" name="pago_html_inferior"
                        id="pago_html_inferior">{{ old('pago_html_inferior', '') }}</textarea>
                </div>
                <div class="form-group">
                    <label for="parte_viajero_html">{{ trans('cruds.sociedad.fields.parte_viajero_html') }} (legacy)</label>
                    <textarea class="form-control {{ $errors->has('parte_viajero_html') ? 'is-invalid' : '' }}" name="parte_viajero_html"
                        id="parte_viajero_html">{{ old('parte_viajero_html', '') }}</textarea>
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control {{ $errors->has('parte_viajero_html_es') ? 'is-invalid' : '' }}" name="parte_viajero_html_es"
                        id="parte_viajero_html_es">{{ old('parte_viajero_html_es', '') }}</textarea>
                </div>
                <div class="form-group">
                    <label for="parte_viajero_html_en">Travellers form (EN)</label>
                    <textarea class="form-control {{ $errors->has('parte_viajero_html_en') ? 'is-invalid' : '' }}" name="parte_viajero_html_en"
                        id="parte_viajero_html_en">{{ old('parte_viajero_html_en', '') }}</textarea>
                </div>
                <div class="row">
                    <div class="col-md-12">
                        <div class="alert alert-info" style="color:#0c5460;background:#d1ecf1;border-color:#bee5eb">
                            ...
                            <ul style="margin-top:8px">
                                <li>Orden de uso: EN (si el idioma activo es inglés) - ES - texto legacy.</li>
                                <li>Formato: puedes introducir HTML (párrafos, listas, enlaces). Se renderiza tal cual en el PDF.</li>
                                <li>Variables: estos textos no procesan variables. Para textos dinámicos, personaliza la cabecera por Sociedad en <code>resources/views/admin/pdf/sociedades/{sociedad_id}/parte_header.blade.php</code>. Si no existe, se usa la genérica.</li>
                            </ul>
                            <p style="margin:6px 0 0 0"><small>En la cabecera personalizada tendrás disponibles: <code>$establecimiento</code>, <code>$sociedad</code>, <code>$reservation</code> y <code>$checkin</code>. Puedes copiar como base <code>resources/views/admin/pdf/parte_header.blade.php</code>.</small></p>
                        </div>
                    </div>
                </div>
                <div class="row">
                    <div class="col-3">
                        <div class="form-group">
                            ...
                            {!! UtilService::drowDownList('show_btns_capture_document', $show_btns_capture_document_list, '', [
                                'id' => 'show_btns_capture_document',
                                'class' => 'form-control',
