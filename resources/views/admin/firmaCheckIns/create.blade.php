...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.firma-check-ins.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label class="required" for="sesion_id">{{ trans('cruds.firmaCheckIn.fields.sesion') }}</label>
                <select class="form-control select2 {{ $errors->has('sesion') ? 'is-invalid' : '' }}" name="sesion_id" id="sesion_id" required>
