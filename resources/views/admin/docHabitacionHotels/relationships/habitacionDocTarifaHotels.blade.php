@can('doc_tarifa_hotel_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.doc-tarifa-hotels.create') }}">
                        ...
                        <tr data-entry-id="{{ $docTarifaHotel->id }}">
                            ...
                            <td>
                                @can('doc_tarifa_hotel_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.doc-tarifa-hotels.show', $docTarifaHotel->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('doc_tarifa_hotel_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.doc-tarifa-hotels.edit', $docTarifaHotel->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('doc_tarifa_hotel_delete')
                                    <form action="{{ route('admin.doc-tarifa-hotels.destroy', $docTarifaHotel->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('doc_tarifa_hotel_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.doc-tarifa-hotels.massDestroy') }}",
