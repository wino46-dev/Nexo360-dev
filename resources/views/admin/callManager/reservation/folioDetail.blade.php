                        ...
                        @if (isset($origen) && $origen == 'folio_selected')
                            <td class="text-nowrap text-center" style="width:130px">
                                <button class="btn btn-success btn-sm"
                                    ...
                                    data-folio='@json(["id" => $folio["id"] ?? null, "name" => $folio["name"] ?? null])'
        ...
        <script>
            (function(){
                try{
                    ...
                    console.log('[ADMIN] folioDetail: reservas con servicios =', {!! json_encode($reservationsWithServices->pluck('id')) !!}, 'firstId=', firstId);
                    if(hasAny && firstId){
                        $('#fixed_costs_section').show();
                        load_fixed_costs(firstId);
                    } else {
                        console.log('[ADMIN] folioDetail: no hay reservas con servicios, ocultando sección costes');
                    }
                }catch(e){ console.warn('[ADMIN] folioDetail init error', e); }

                window.load_fixed_costs = function(reservation_id){
                    var $c = $('#fixed_costs_container');
                    console.log('[ADMIN] load_fixed_costs ->', reservation_id);
                    $c.html('<div class="py-2 text-center">'+ (window.loading1 || 'Cargando...') +'</div>');
                    $.ajax({
                        url: "{{ url((request()->is('admin*') ? 'admin' : 'external').'/reservation-api/reservation-costs') }}/" + reservation_id + '?embed=1',
                        method: 'GET',
                        cache: false
                    }).done(function(html){
                        console.log('[ADMIN] load_fixed_costs OK for', reservation_id);
                        $c.html(html);
                        ...
                        console.error('[ADMIN] load_fixed_costs FAIL for', reservation_id, xhr);
