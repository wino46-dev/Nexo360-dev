...
@endunless
@include('admin.callManager.reservation.resumenModal')
{{-- @include('admin.alice.modal', ['control_actual' => $control_actual]) --}}
@unless(request()->is('external*'))
@include('admin.sh360-ocr.modal', ['control_actual' => $control_actual])
@endunless
...
<script>
    ...
    // Neutralizar funciones de acción en External para evitar llamadas a rutas admin
    ...
    function checkin_confirm($this, tipo) {
        ...
        if (tipo == 'validate_pago') {
            $.ajax({
                url: '{{ url((request()->is('external*')?'external':'admin').'/reservation-api/checkin') }}?opc=validate_pago&embed=1',
                method: 'POST',
                cache: false,
                headers: {
        ...
        if (tipo == 'validate') {
            $.ajax({
                url: '{{ url((request()->is('external*')?'external':'admin').'/reservation-api/checkin') }}?opc=validate&embed=1',
                method: 'POST',
                cache: false,
                headers: {
                    'X-CSRF-TOKEN': $('input[name="_token"]').val()
            ...
            $.ajax({
                url: '{{ url((request()->is('external*')?'external':'admin').'/reservation-api/checkin') }}?opc=save&embed=1',
                method: 'POST',
                cache: false,
                headers: {
                    'X-CSRF-TOKEN': $('input[name="_token"]').val()
    ...
    function foto_documento_load(id, tipo) {
        ...
        $.ajax({
            url: "{{ route('admin.call-manager.documentView') }}",
            method: 'POST',
            //dataType: 'JSON',
            //  contentType: false,
            cache: false,
    ...
    function foto_firma_load(id) {
        ...
        $.ajax({
            url: "{{ route('admin.call-manager.signView') }}",
            method: 'POST',
            //dataType: 'JSON',
            //  contentType: false,
            cache: false,
    ...
    function evento_totem(id, tipo, form_id) {
        ...
        $.ajax({
            url: "{{ route($panelPrefix . '.evento-home-totems.store') }}",
            method: 'POST',
            cache: false,
            headers: {
                'X-CSRF-TOKEN': $('input[name="_token"]').val()
        ...
        $(document).ready(function() {
            $('#partner_form_{{ $key }}').validate({
                ...
                submitHandler: function(form) {
                    ...
                    $.ajax({
                        url: '{{ url((request()->is('external*')?'external':'admin').'/reservation-api/checkin-partner') }}?embed=1',
                        method: 'POST',
                        data: formData,
                    }).done(function(data) {
                        console.log(data);
                        ...
                        if (partner_form_action == 'checkin_save') {
                            ...
                            try {
                                if (typeof window.load_fixed_costs === 'function' && data && data.reservation_id) {
                                    console.log('[ADMIN] Refrescando costes tras checkin parcial para', data.reservation_id);
    ...
    function countries_states(country_id, select_to) {
        ...
        $.ajax({
            url: '{{ url((request()->is('external*')?'external':'admin').'/reservation-api/countries-states') }}/' + country_id + '?embed=1',
            method: 'GET',
            dataType: 'json'
        }).done(function(data) {
            select_container.empty();
    ...
    function huesped_relacion_load(reservation_id) {
        $.ajax({
            url: '{{ url('admin/call-manager/huesped-relacion') }}',
            method: 'POST',
    ...
    function client_search(partner_id, key) {
        ...
        $.ajax({
            url: "{{ url($panelPrefix . '/call-manager/huesped-info') }}",
            method: 'POST',
            cache: false,
            headers: {
                'X-CSRF-TOKEN': $('input[name="_token"]').val()
