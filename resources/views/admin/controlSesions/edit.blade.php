...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.control-sesions.update", [$controlSesion->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('estado_sesion') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="estado_sesion" id="estado_sesion" value="1" {{ $controlSesion->estado_sesion || old('estado_sesion', 0) === 1 ? 'checked' : '' }}>
