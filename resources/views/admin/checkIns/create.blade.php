...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route(((auth()->check() && method_exists(auth()->user(), 'isExternal') && auth()->user()->isExternal()) ? 'external' : 'admin') . '.check-ins.store') }}" enctype="multipart/form-data">
            @csrf
