...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route(((auth()->check() && method_exists(auth()->user(), 'isExternal') && auth()->user()->isExternal()) ? 'external' : 'admin') . '.check-ins.update', [$checkIn->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('cliente') ? 'is-invalid' : '' }}" name="cliente_id" id="cliente_id" required>
                    @foreach($clientes as $id => $entry)
                        <option value="{{ $id }}" {{ (old('cliente_id') ? old('cliente_id') : $checkIn->cliente->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('reserva') ? 'is-invalid' : '' }}" name="reserva_id" id="reserva_id" required>
                    @foreach($reservas as $id => $entry)
                        <option value="{{ $id }}" {{ (old('reserva_id') ? old('reserva_id') : $checkIn->reserva->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('habitacion') ? 'is-invalid' : '' }}" name="habitacion_id" id="habitacion_id">
                    @foreach($habitacions as $id => $entry)
                        <option value="{{ $id }}" {{ (old('habitacion_id') ? old('habitacion_id') : $checkIn->habitacion->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('totem') ? 'is-invalid' : '' }}" name="totem_id" id="totem_id" required>
                    @foreach($totems as $id => $entry)
                        <option value="{{ $id }}" {{ (old('totem_id') ? old('totem_id') : $checkIn->totem->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('totem'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('llave') ? 'is-invalid' : '' }}" type="text" name="llave" id="llave" value="{{ old('llave', $checkIn->llave) }}">
                @if($errors->has('llave'))
                    <div class="invalid-feedback">
                        {{ $errors->first('llave') }}
                    </div>
            ...
            <div class="form-group">
                <label for="firma_checkin">{{ trans('cruds.checkIn.fields.firma_checkin') }}</label>
                <div class="needsclick dropzone {{ $errors->has('firma_checkin') ? 'is-invalid' : '' }}" id="firma_checkin-dropzone">
                </div>
                @if($errors->has('firma_checkin'))
                    <div class="invalid-feedback">
                        {{ $errors->first('firma_checkin') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.checkIn.fields.firma_checkin_helper') }}</span>
            </div>
            <div class="form-group">
                <div class="form-check {{ $errors->has('firma_verificada') ? 'is-invalid' : '' }}">
                    <input type="hidden" name="firma_verificada" value="0">
                    <input class="form-check-input" type="checkbox" name="firma_verificada" id="firma_verificada" value="1" {{ $checkIn->firma_verificada || old('firma_verificada', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('comentarios') ? 'is-invalid' : '' }}" name="comentarios" id="comentarios">{!! old('comentarios', $checkIn->comentarios) !!}</textarea>
...
<script>
    ...
    url: '{{ route(((auth()->check() && method_exists(auth()->user(), 'isExternal') && auth()->user()->isExternal()) ? 'external' : 'admin') . '.check-ins.storeMedia') }}',
    maxFilesize: 8, // MB
...
<script>
    Dropzone.options.firmaCheckinDropzone = {
    url: '{{ route(((auth()->check() && method_exists(auth()->user(), 'isExternal') && auth()->user()->isExternal()) ? 'external' : 'admin') . '.check-ins.storeMedia') }}',
    maxFilesize: 4, // MB
    acceptedFiles: '.jpeg,.jpg,.png,.gif',
    maxFiles: 1,
    ...
    success: function (file, response) {
      $('form').find('input[name="firma_checkin"]').remove()
      $('form').append('<input type="hidden" name="firma_checkin" value="' + response.name + '">')
    },
    removedfile: function (file) {
      file.previewElement.remove()
      if (file.status !== 'error') {
    ...
    init: function () {
@if(isset($checkIn) && $checkIn->firma_checkin)
      var file = {!! json_encode($checkIn->firma_checkin) !!}
          this.options.addedfile.call(this, file)
      this.options.thumbnail.call(this, file, file.preview ?? file.preview_url)
      file.previewElement.classList.add('dz-complete')
      $('form').append('<input type="hidden" name="firma_checkin" value="' + file.file_name + '">')
      this.options.maxFiles = this.options.maxFiles - 1
@endif
    },
    error: function (file, response) {
          ...
          return loader.file
            .then(function (file) {
              return new Promise(function(resolve, reject) {
                ...
                xhr.open('POST', '{{ route(((auth()->check() && method_exists(auth()->user(), 'isExternal') && auth()->user()->isExternal()) ? 'external' : 'admin') . '.check-ins.storeCKEditorImages') }}', true);
