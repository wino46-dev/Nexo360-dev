        ...
        <div class='col-md-12'>
            <div class="card panel-default">
                ...
                <div class="card-body table-responsive">
                    <form class="form-horizontal" method="POST" action="{{ $config['url'] }}/process-csv-import">
                        {{ csrf_field() }}
                                    ...
                                    <td>
                                        <select name="fields[{{ $key }}]">
                                            ...
                                            @foreach($fillables as $k => $fillable)
                                                <option
                                                    value="{{ $fillable }}" {{ strtolower($header) === strtolower($fillable) ? 'selected' : '' }}>{{ $fillable }}</option>
                                            @endforeach
