...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.parteViajero.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.parte-viajeros.update", [$parteViajero->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('reserva') ? 'is-invalid' : '' }}" name="reserva_id" id="reserva_id" required>
                    @foreach($reservas as $id => $entry)
                        <option value="{{ $id }}" {{ (old('reserva_id') ? old('reserva_id') : $parteViajero->reserva->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('cliente') ? 'is-invalid' : '' }}" name="cliente_id" id="cliente_id" required>
                    @foreach($clientes as $id => $entry)
                        <option value="{{ $id }}" {{ (old('cliente_id') ? old('cliente_id') : $parteViajero->cliente->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('texto_inferior') ? 'is-invalid' : '' }}" name="texto_inferior" id="texto_inferior">{{ old('texto_inferior', $parteViajero->texto_inferior) }}</textarea>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('firma') ? 'is-invalid' : '' }}" name="firma" id="firma">{{ old('firma', $parteViajero->firma) }}</textarea>
                @if($errors->has('firma'))
                    <div class="invalid-feedback">
                        {{ $errors->first('firma') }}
                    </div>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('envio_mail') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="envio_mail" id="envio_mail" value="1" {{ $parteViajero->envio_mail || old('envio_mail', 0) === 1 ? 'checked' : '' }}>
