...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.respuestaEventoHomeTotem.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.respuesta-evento-home-totems.update", [$respuestaEventoHomeTotem->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('evento') ? 'is-invalid' : '' }}" name="evento_id" id="evento_id" required>
                    @foreach($eventos as $id => $entry)
                        <option value="{{ $id }}" {{ (old('evento_id') ? old('evento_id') : $respuestaEventoHomeTotem->evento->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('respuesta') ? 'is-invalid' : '' }}" name="respuesta" id="respuesta" required>{{ old('respuesta', $respuestaEventoHomeTotem->respuesta) }}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('estado') ? 'is-invalid' : '' }}" name="estado" id="estado" required>
                    ...
                    @foreach(App\Models\RespuestaEventoHomeTotem::ESTADO_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('estado', $respuestaEventoHomeTotem->estado) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
