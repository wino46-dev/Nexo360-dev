...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.docIncidenciaHotel.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.doc-incidencia-hotels.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('fecha') ? 'is-invalid' : '' }}" type="datetime-local" id="fecha_dt" required>
            ...
            <script>
                document.addEventListener('DOMContentLoaded', function(){
                    ...
                    // Prefill visible from hidden (old value in backend format)
                    if (hidden.value) {
                        var m = moment(hidden.value, 'DD-MM-YYYY HH:mm:ss', true);
                        if (!m.isValid()) {
                            // try ISO as fallback
                            m = moment(hidden.value);
                        }
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('estado') ? 'is-invalid' : '' }}" name="estado" id="estado" required>
                    ...
                    @foreach(App\Models\DocIncidenciaHotel::ESTADO_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('estado', 'Abierta') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
          ...
          return loader.file
            .then(function (file) {
              return new Promise(function(resolve, reject) {
                ...
                xhr.open('POST', '{{ route('admin.doc-incidencia-hotels.storeCKEditorImages') }}', true);
