...
@include('admin.manager.captureClient')
...
<script type="text/javascript">
    // Simple polling helper to fetch document images with retries
    function pollImage(side, maxAttempts = 10, delayMs = 2000) {
        let attempts = 0;
        const form = document.getElementById('refreshDivCaptureForm');
        const formData = new FormData(form);
        const doPoll = function() {
            $.ajax({
                url: "{{ route("admin.evento-home-totems.updateContentDocument") }}",
                method: 'POST',
                dataType: 'JSON',
                contentType: false,
                cache: false,
                ...
                success: function(response) {
                    if (response && response[side]) {
                        ...
                        } else if (side === 'reverso') {
                            $(".respuestaReversoPush").empty();
                            $(".respuestaReversoPush").append('<br><h5>Parte trasera del documento</h5>');
                    ...
                    if (attempts < maxAttempts) {
                        setTimeout(doPoll, delayMs);
                    } else {
                        $("#error-alert-push-documento").show().html('<strong>Ups! </strong>No se recibió la imagen de ' + side + ' tras varios intentos.');
                        setTimeout(function(){ $("#error-alert-push-documento").fadeOut(); }, 4000);
                    }
                },
                error: function() {
                    attempts++;
                    if (attempts < maxAttempts) {
                        ...
                        $("#error-alert-push-documento").show().html('<strong>Ups! </strong>Error consultando el estado del documento.');
                        setTimeout(function(){ $("#error-alert-push-documento").fadeOut(); }, 4000);
                    }
                }
            });
    ...
    $(document).ready(function(){
        ...
        $('#fotoAnverso').on('submit',function(e){
            e.preventDefault();
            $.ajax({
                url: "{{ route("admin.evento-home-totems.store") }}",
                method: 'POST',
                dataType: 'JSON',
                contentType: false,
                cache: false,
                ...
                success:function(response){
                    ...
                    if(response['success']){
                        ...
                        // Iniciar polling para el anverso
                        if (typeof pollImage === 'function') {
                            pollImage('anverso', 10, 2000);
                        }
            ...
            $.ajax({
                ...
                success:function(response){
                    ...
                    if(response['success']){
                        ...
                        // Iniciar polling para el reverso
                        if (typeof pollImage === 'function') {
                            pollImage('reverso', 10, 2000);
                        }
                        console.log(response)
                    }
                    if(response['error']){
