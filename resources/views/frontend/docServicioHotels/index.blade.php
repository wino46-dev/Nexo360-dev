        ...
        <div class="col-md-12">
            @can('doc_servicio_hotel_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.doc-servicio-hotels.create') }}">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table class=" table table-bordered table-striped table-hover datatable datatable-DocServicioHotel">
                            <thead>
                                <tr>
                                    <th>
                            ...
                            <tbody>
                                @foreach($docServicioHotels as $key => $docServicioHotel)
                                    <tr data-entry-id="{{ $docServicioHotel->id }}">
                                        <td>
                                            {{ $docServicioHotel->id ?? '' }}
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            <span style="display:none">{{ $docServicioHotel->por_persona ?? '' }}</span>
                                            <input type="checkbox" disabled="disabled" {{ $docServicioHotel->por_persona ? 'checked' : '' }}>
                                        </td>
                                        <td>
                                            <span style="display:none">{{ $docServicioHotel->por_dia ?? '' }}</span>
                                        ...
                                        <td>
                                            @can('doc_servicio_hotel_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.doc-servicio-hotels.show', $docServicioHotel->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('doc_servicio_hotel_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.doc-servicio-hotels.edit', $docServicioHotel->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('doc_servicio_hotel_delete')
                                                <form action="{{ route('frontend.doc-servicio-hotels.destroy', $docServicioHotel->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('doc_servicio_hotel_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.doc-servicio-hotels.massDestroy') }}",
...
@endcan
  ...
  let table = $('.datatable-DocServicioHotel:not(.ajaxTable)').DataTable({ buttons: dtButtons })
