        ...
        <div class="col-md-12">
            ...
            <div class="card">
                <div class="card-header">
                    {{ trans('global.create') }} {{ trans('cruds.firmaCheckIn.title_singular') }}
                </div>

                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.firma-check-ins.store") }}" enctype="multipart/form-data">
                        @method('POST')
                        @csrf
                        <div class="form-group">
                            <label class="required" for="sesion_id">{{ trans('cruds.firmaCheckIn.fields.sesion') }}</label>
                            <select class="form-control select2" name="sesion_id" id="sesion_id" required>
...
<script>
    ...
    url: '{{ route('frontend.firma-check-ins.storeMedia') }}',
    maxFilesize: 4, // MB
    maxFiles: 1,
    addRemoveLinks: true,
    headers: {
