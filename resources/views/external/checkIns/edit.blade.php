...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route(((auth()->check() && method_exists(auth()->user(), 'isExternal') && auth()->user()->isExternal()) ? 'external' : 'external') . '.check-ins.update', [$checkIn->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
