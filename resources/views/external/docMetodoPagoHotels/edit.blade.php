...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.docMetodoPagoHotel.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("external.doc-metodo-pago-hotels.update", [$docMetodoPagoHotel->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('establecimiento') ? 'is-invalid' : '' }}" name="establecimiento_id" id="establecimiento_id" required>
                    @foreach($establecimientos as $id => $entry)
                        <option value="{{ $id }}" {{ (old('establecimiento_id') ? old('establecimiento_id') : $docMetodoPagoHotel->establecimiento->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('nombre') ? 'is-invalid' : '' }}" type="text" name="nombre" id="nombre" value="{{ old('nombre', $docMetodoPagoHotel->nombre) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('tipo') ? 'is-invalid' : '' }}" name="tipo" id="tipo">
                    ...
                    @foreach(App\Models\DocMetodoPagoHotel::TIPO_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('tipo', $docMetodoPagoHotel->tipo) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('tipo'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('activo') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="activo" id="activo" value="1" {{ $docMetodoPagoHotel->activo || old('activo', 0) === 1 ? 'checked' : '' }}>
                    <label class="form-check-label" for="activo">{{ trans('cruds.docMetodoPagoHotel.fields.activo') }}</label>
                </div>
