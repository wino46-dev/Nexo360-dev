...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.docHotelEstadoCaja.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("external.doc-hotel-estado-cajas.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('caja') ? 'is-invalid' : '' }}" type="number" name="caja" id="caja" value="{{ old('caja', '') }}" step="1" required>
                @if($errors->has('caja'))
                    <div class="invalid-feedback">
                        {{ $errors->first('caja') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('room_status') ? 'is-invalid' : '' }}" name="room_status" id="room_status">
                    ...
                    @foreach(App\Models\DocHotelEstadoCaja::ROOM_STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('room_status', 'CI') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('cliente') ? 'is-invalid' : '' }}" type="text" name="cliente" id="cliente" value="{{ old('cliente', '') }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('documento_cliente') ? 'is-invalid' : '' }}" type="text" name="documento_cliente" id="documento_cliente" value="{{ old('documento_cliente', '') }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('pay') ? 'is-invalid' : '' }}" name="pay" id="pay">
                    ...
                    @foreach(App\Models\DocHotelEstadoCaja::PAY_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('pay', 'No Cobrar') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('pay'))
                    <div class="invalid-feedback">
          ...
          return loader.file
            .then(function (file) {
              return new Promise(function(resolve, reject) {
                ...
                xhr.open('POST', '{{ route('external.doc-hotel-estado-cajas.storeCKEditorImages') }}', true);
