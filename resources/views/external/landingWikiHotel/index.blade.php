...
<script>
    ...
    function bindAjaxForm($modal, tableReloadFn){
        ...
        function normalizeActionUrl(url){
            try {
                ...
                // Fuerza siempre external
                u.pathname = url.pathname.replace('/external/', '/external/');
                return u.toString();
            } catch(e) {
                // Si no puede parsear bien la URL, también fuerza el reemplazo
                return (url || '').replace('/external/', '/external/');
            }
        }
    ...
    function openAjaxModal(url, tableReloadFn, isShow){
        ...
        (function(){ var __u = (function(u){ try { var tmp = new URL(u, window.location.origin); tmp.searchParams.set('embed','1'); return tmp.toString(); } catch(e){ return u; } })(url); $.get(__u, function(html){
            ...
            if (isControlSesion) {
                var managerUrl = "{{ route(($panelPrefix ?? (request()->is('external*') ? 'external' : 'external')) . '.control-sesions.manager') }}";
    ...
    var EDIT_RECEPCION_URL_TMPL = "{{ route(((auth()->check() && method_exists(auth()->user(),'isExternal') && auth()->user()->isExternal()) ? 'external' : 'external') . '.doc-hotel-reception-infos.edit', ['doc_hotel_reception_info' => '__ID__']) }}";
    var EDIT_INFO_URL_TMPL = "{{ route(((auth()->check() && method_exists(auth()->user(),'isExternal') && auth()->user()->isExternal()) ? 'external' : 'external') . '.doc-info-hotels.edit', ['doc_info_hotel' => '__ID__']) }}";
    ...
    function initRecepcionTable(hotelId){
        ...
        recepcionTable = $('#tablaRecepcion').DataTable({
            ...
            aaSorting: [],
            ajax: {
                url: "{{ route(((auth()->check() && method_exists(auth()->user(),'isExternal') && auth()->user()->isExternal()) ? 'external' : 'external') . '.doc-hotel-reception-infos.index') }}",
    ...
    function initEstadoCajaTable(hotelId){
        ...
        estadoCajaTable = $('#tablaEstadoCaja').DataTable({
            ...
            aaSorting: [],
            ajax: {
                url: "{{ route(((auth()->check() && method_exists(auth()->user(),'isExternal') && auth()->user()->isExternal()) ? 'external' : 'external') . '.doc-hotel-estado-cajas.index') }}",
                data: function(d){ d.establecimiento_id = window.WikiHotelSelectedId || $('#selectorHotel').val(); }
            },
            columns: [
                { data: 'placeholder', name: 'placeholder' },
    ...
    function initInfoHotelTable(hotelId){
        ...
        infoHotelTable = $('#tablaInfoHotel').DataTable({
            ...
            aaSorting: [],
            ajax: {
                url: "{{ route(((auth()->check() && method_exists(auth()->user(),'isExternal') && auth()->user()->isExternal()) ? 'external' : 'external') . '.doc-info-hotels.index') }}",
                data: function(d){ d.establecimiento_id = window.WikiHotelSelectedId || $('#selectorHotel').val(); }
            },
            columns: [
                { data: 'placeholder', name: 'placeholder' },
    ...
    function initServicioHotelTable(hotelId){
        ...
        servicioHotelTable = $('#tablaServicioHotel').DataTable({
            ...
            aaSorting: [],
            ajax: {
                url: "{{ route(((auth()->check() && method_exists(auth()->user(),'isExternal') && auth()->user()->isExternal()) ? 'external' : 'external') . '.doc-servicio-hotels.index') }}",
                data: function(d){ d.establecimiento_id = window.WikiHotelSelectedId || $('#selectorHotel').val(); }
            },
            columns: [
                { data: 'placeholder', name: 'placeholder' },
    ...
    function initMetodoPagoHotelTable(hotelId){
        ...
        metodoPagoHotelTable = $('#tablaMetodoPagoHotel').DataTable({
            ...
            aaSorting: [],
            ajax: {
                url: "{{ route(((auth()->check() && method_exists(auth()->user(),'isExternal') && auth()->user()->isExternal()) ? 'external' : 'external') . '.doc-metodo-pago-hotels.index') }}",
                data: function(d){ d.establecimiento_id = window.WikiHotelSelectedId || $('#selectorHotel').val(); }
            },
            columns: [
                { data: 'placeholder', name: 'placeholder' },
    ...
    function initUbicacionHotelTable(hotelId){
        ...
        ubicacionHotelTable = $('#tablaUbicacionHotel').DataTable({
            ...
            aaSorting: [],
            ajax: {
                url: "{{ route(((auth()->check() && method_exists(auth()->user(),'isExternal') && auth()->user()->isExternal()) ? 'external' : 'external') . '.doc-ubicacion-hotels.index') }}",
                data: function(d){ d.establecimiento_id = window.WikiHotelSelectedId || $('#selectorHotel').val(); }
            },
            columns: [
                { data: 'placeholder', name: 'placeholder' },
    ...
    function initStockHotelTable(hotelId){
        ...
        stockHotelTable = $('#tablaStockHotel').DataTable({
            ...
            aaSorting: [],
            ajax: {
                url: "{{ route(((auth()->check() && method_exists(auth()->user(),'isExternal') && auth()->user()->isExternal()) ? 'external' : 'external') . '.dock-stock-hotels.index') }}",
                data: function(d){ d.establecimiento_id = window.WikiHotelSelectedId || $('#selectorHotel').val(); }
            },
            columns: [
                { data: 'placeholder', name: 'placeholder' },
    ...
    function initIncidenciaHotelTable(hotelId){
        ...
        incidenciaHotelTable = $('#tablaIncidenciaHotel').DataTable({
            ...
            aaSorting: [],
            ajax: {
                url: "{{ route(((auth()->check() && method_exists(auth()->user(),'isExternal') && auth()->user()->isExternal()) ? 'external' : 'external') . '.doc-incidencia-hotels.index') }}",
                data: function(d){ d.establecimiento_id = window.WikiHotelSelectedId || $('#selectorHotel').val(); }
            },
            columns: [
                { data: 'placeholder', name: 'placeholder' },
    ...
    function initNoDeseadoHotelTable(hotelId){
        ...
        noDeseadoHotelTable = $('#tablaNoDeseadoHotel').DataTable({
            ...
            aaSorting: [],
            ajax: {
                url: "{{ route(((auth()->check() && method_exists(auth()->user(),'isExternal') && auth()->user()->isExternal()) ? 'external' : 'external') . '.doc-no-deseado-hotels.index') }}",
                data: function(d){ d.establecimiento_id = window.WikiHotelSelectedId || $('#selectorHotel').val(); }
            },
            columns: [
                { data: 'placeholder', name: 'placeholder' },
    ...
    function initHabitacionHotelTable(hotelId){
        ...
        habitacionHotelTable = $('#tablaHabitacionHotel').DataTable({
            ...
            aaSorting: [],
            ajax: {
                url: "{{ route(((auth()->check() && method_exists(auth()->user(),'isExternal') && auth()->user()->isExternal()) ? 'external' : 'external') . '.doc-habitacion-hotels.index') }}",
                data: function(d){ d.establecimiento_id = window.WikiHotelSelectedId || $('#selectorHotel').val(); }
            },
            columns: [
                { data: 'placeholder', name: 'placeholder' },
    ...
    function initTarifaHotelTable(hotelId){
        ...
        tarifaHotelTable = $('#tablaTarifaHotel').DataTable({
            ...
            aaSorting: [],
            ajax: {
                url: "{{ route(((auth()->check() && method_exists(auth()->user(),'isExternal') && auth()->user()->isExternal()) ? 'external' : 'external') . '.doc-tarifa-hotels.index') }}",
    ...
    $(function(){
        ...
        $('#btn-create-recepcion').on('click', function(e){
            ...
            var sel = getSelectedHotel();
            if(!sel.id) return false;
            var url = new URL("{{ route('external.doc-hotel-reception-infos.create') }}", window.location.origin);
            url.searchParams.set('establecimiento_id', sel.id);
        ...
        $('#btn-create-estado-caja').on('click', function(e){
            ...
            var sel = getSelectedHotel();
            if(!sel.id) return false;
            var url = new URL("{{ route('external.doc-hotel-estado-cajas.create') }}", window.location.origin);
            url.searchParams.set('establecimiento_id', sel.id);
        ...
        $('#btn-create-info-hotel').on('click', function(e){
            e.preventDefault(); e.stopPropagation(); if (e.stopImmediatePropagation) e.stopImmediatePropagation();
            var sel = getSelectedHotel(); if(!sel.id) return false;
            var url = new URL("{{ route('external.doc-info-hotels.create') }}", window.location.origin);
            url.searchParams.set('establecimiento_id', sel.id);
        ...
        $('#btn-create-servicio-hotel').on('click', function(e){
            e.preventDefault(); e.stopPropagation(); if (e.stopImmediatePropagation) e.stopImmediatePropagation();
            var sel = getSelectedHotel(); if(!sel.id) return false;
            var url = new URL("{{ route('external.doc-servicio-hotels.create') }}", window.location.origin);
            url.searchParams.set('establecimiento_id', sel.id);
        ...
        $('#btn-create-metodo-pago-hotel').on('click', function(e){
            e.preventDefault(); e.stopPropagation(); if (e.stopImmediatePropagation) e.stopImmediatePropagation();
            var sel = getSelectedHotel(); if(!sel.id) return false;
            var url = new URL("{{ route('external.doc-metodo-pago-hotels.create') }}", window.location.origin);
            url.searchParams.set('establecimiento_id', sel.id);
        ...
        $('#btn-create-ubicacion-hotel').on('click', function(e){
            e.preventDefault(); e.stopPropagation(); if (e.stopImmediatePropagation) e.stopImmediatePropagation();
            var sel = getSelectedHotel(); if(!sel.id) return false;
            var url = new URL("{{ route('external.doc-ubicacion-hotels.create') }}", window.location.origin);
            url.searchParams.set('establecimiento_id', sel.id);
        ...
        $('#btn-create-stock-hotel').on('click', function(e){
            e.preventDefault(); e.stopPropagation(); if (e.stopImmediatePropagation) e.stopImmediatePropagation();
            var sel = getSelectedHotel(); if(!sel.id) return false;
            var url = new URL("{{ route('external.dock-stock-hotels.create') }}", window.location.origin);
            url.searchParams.set('establecimiento_id', sel.id);
        ...
        $('#btn-create-incidencia-hotel').on('click', function(e){
            e.preventDefault(); e.stopPropagation(); if (e.stopImmediatePropagation) e.stopImmediatePropagation();
            var sel = getSelectedHotel(); if(!sel.id) return false;
            var url = new URL("{{ route('external.doc-incidencia-hotels.create') }}", window.location.origin);
            url.searchParams.set('establecimiento_id', sel.id);
        ...
        $('#btn-create-no-deseado-hotel').on('click', function(e){
            e.preventDefault(); e.stopPropagation(); if (e.stopImmediatePropagation) e.stopImmediatePropagation();
            var sel = getSelectedHotel(); if(!sel.id) return false;
            var url = new URL("{{ route('external.doc-no-deseado-hotels.create') }}", window.location.origin);
            url.searchParams.set('establecimiento_id', sel.id);
        ...
        $('#btn-create-habitacion-hotel').on('click', function(e){
            e.preventDefault(); e.stopPropagation(); if (e.stopImmediatePropagation) e.stopImmediatePropagation();
            var sel = getSelectedHotel(); if(!sel.id) return false;
            var url = new URL("{{ route('external.doc-habitacion-hotels.create') }}", window.location.origin);
            url.searchParams.set('establecimiento_id', sel.id);
        ...
        $('#btn-create-tarifa-hotel').on('click', function(e){
            e.preventDefault(); e.stopPropagation(); if (e.stopImmediatePropagation) e.stopImmediatePropagation();
            var sel = getSelectedHotel(); if(!sel.id) return false;
            var url = new URL("{{ route('external.doc-tarifa-hotels.create') }}", window.location.origin);
            url.searchParams.set('establecimiento_id', sel.id);
