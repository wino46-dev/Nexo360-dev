...
<script>
    ...
    function folioStatus() {
        ...
        $.ajax({
            url: "{{ url((request()->is('external*')?'external':'external').'/reservation-api/folio-status') }}?embed=1",
            method: 'GET',
            cache: false,
            headers: {
    ...
    function folio_search(type) {
        ...
        if (type == 'search') {
            ...
            url = '{{ url((request()->is('external*')?'external':'external').'/reservation-api/folio-search') }}?q=' + input_data + url_filter_date + '&embed=1';
        } else {
            url = '{{ url((request()->is('external*')?'external':'external').'/reservation-api/folio-search') }}?type=' + type + '&embed=1';
        }
    ...
    function showFolioDetail(id) {
        $.ajax({
            url: "{{ url((request()->is('external*')?'external':'external').'/reservation-api/folio-detail-select') }}?embed=1",
            method: 'POST',
            cache: false,
            headers: {
                'X-CSRF-TOKEN': $('input[name="_token"]').val()
...
@include('external.callManager.reservation.folioDetailModal')
