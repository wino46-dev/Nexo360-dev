...
@section('content')
    ...
    @if (isset($control_actual))
        @include('external.callManager.styles')
        ...
        @if ($pms_active)
            ...
            @include('external.callManager.reservation.folioSearch')
                ...
                <div class="card-body p-3">
                    ...
                    <div class="tab-content" id="myTabContent">
                        <div class="tab-pane fade show active bg-white" id="home" role="tabpanel"
                            ...
                            <div class="row">
                                <div class="col-md-5 pt-2" id="checkingContainerLeft">
                                    @include('external.callManager.payment')

                                    @unless(request()->is('external*'))
                                    @include('external.callManager.writeCard', ['id' => 1])
                                    @endunless

                                    @include('external.callManager.photoView')
                        ...
                        <div class="tab-pane fade bg-white " id="profile" role="tabpanel" aria-labelledby="profile-tab">
                            @include('external.callManager.imagesIndex')
                        </div>
                        @endunless
                    </div>
                </div>
        ...
        @unless(request()->is('external*'))
        @include('external.callManager.writeCardModal')
        @endunless

        {{-- Modal Incidencias abiertas del hotel --}}
        <div class="modal fade" id="modalIncidenciasHotel" tabindex="-1" role="dialog" aria-labelledby="incidenciasLabel" aria-hidden="true">
...
@section('scripts')
    <script>
        ...
        window.nombreEstablecimiento = @json(isset($establecimiento) && isset($establecimiento->nombre) ? $establecimiento->nombre : null);
            ...
            try {
                if (Array.isArray(window.incidenciasAbiertas) && window.incidenciasAbiertas.length > 0) {
                    ...
                    Swal.fire({
                        ...
                        html: (window.nombreEstablecimiento ? '<p><b>Hotel:</b> ' + window.nombreEstablecimiento + '</p>' : '') +
                             '<p>Hemos detectado ' + window.incidenciasAbiertas.length + ' incidencia(s) abierta(s) en este establecimiento.</p>',
        ...
        function reservation_select(id, checkin, checkout) {
            ...
            if (!id) {
                Toast && Toast.fire ? Toast.fire({icon: 'error', title: 'ID de reserva inválido'}) : alert('ID de reserva inválido');
                return;
            }
            ...
            if (checkin) { $('#grabarTarjetaContainer1').find('#date_in').val(checkin); }
            if (checkout) { $('#grabarTarjetaContainer1').find('#date_out').val(checkout); }

            $.ajax({
                url: "{{ url((request()->is('external*')?'external':'external').'/reservation-api/reservation-select') }}/" + id + '?embed=1',
                method: 'GET',
                cache: false,
                ...
                error: function(response) {
                    Toast && Toast.fire ? Toast.fire({icon: 'error', title: 'Error al cargar datos de huéspedes'}) : console.error('Error al cargar datos de huéspedes', response);
                },
            });
        }
