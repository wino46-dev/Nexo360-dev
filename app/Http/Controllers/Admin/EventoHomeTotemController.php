...
class EventoHomeTotemController extends Controller
{
    public function index(Request $request)
    {
        ...
        return view('admin.eventoHomeTotems.index', compact('users', 'tipo_eventos', 'teams'));
    }
    ...
    public function create()
    {
        ...
        return view('admin.eventoHomeTotems.create', compact('receptors', 'tipo_eventos'));
    }
    ...
    public function updateContentDocument(Request $request)
    {
        ...
        $imagen_anverso = EventoHomeTotem::where('sesion_id', $sesionId)
            ->where('tipo_evento_id', 3)->orderBy('id', 'DESC')->first();
        $imagen_reverso = EventoHomeTotem::where('sesion_id', $sesionId)
        ...
        if (isset($imagen_anverso->id)) {
            if (!empty($imagen_anverso->respuesta_texto)) {
                $respuesta['anverso'] = $imagen_anverso->respuesta_texto;
                $anverso_ok = true;
            }
        }
        if (isset($imagen_reverso->id)) {
            if (!empty($imagen_reverso->respuesta_texto)) {
                $respuesta['reverso'] = $imagen_reverso->respuesta_texto;
                $reverso_ok = true;
            }
        }

        Log::info('updateContentDocument polled', [
            'sesion_id' => $sesionId,
            'anverso_event_id' => $imagen_anverso->id ?? null,
            'anverso_has_respuesta' => $anverso_ok,
            'reverso_event_id' => $imagen_reverso->id ?? null,
            'reverso_has_respuesta' => $reverso_ok,
        ]);
    ...
    public function store(StoreEventoHomeTotemRequest $request)
    {
        try {
            if ($request->tipo_evento_id == 3 or $request->tipo_evento_id == 6 or $request->tipo_evento_id == 4) {
                ...
                $eventoHomeTotem = EventoHomeTotem::updateOrCreate($payload, [
                    'respuesta_texto' => $request->respuesta_texto
                ]);

                Log::info('Documento almacenado/updateOrCreate', [
                    'tipo_evento_id' => $request->tipo_evento_id,
                    'sesion_id' => $request->sesion_id,
                    'evento_id' => $eventoHomeTotem->id ?? null,
                    'has_respuesta_texto' => !empty($request->respuesta_texto),
                ]);
            } else {
    ...
    public function edit(EventoHomeTotem $eventoHomeTotem)
    {
        ...
        return view('admin.eventoHomeTotems.edit', compact('eventoHomeTotem', 'tipo_eventos'));
    }
    ...
    public function update(UpdateEventoHomeTotemRequest $request, EventoHomeTotem $eventoHomeTotem)
    {
        ...
        return redirect()->route('admin.evento-home-totems.index');
    }
    ...
    public function show(EventoHomeTotem $eventoHomeTotem)
    {
        ...
        return view('admin.eventoHomeTotems.show', compact('eventoHomeTotem'));
    }
