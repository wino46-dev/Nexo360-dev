...
class ControlSesionController extends Controller
{
    public function index(Request $request)
    {
        ...
        return view('admin.controlSesions.index', compact('users', ));
    }
    ...
    public function create()
    {
        ...
        return view('admin.controlSesions.create', compact('emisors', 'receptors', 'id_sesions','active_sesions'));
    }
    ...
    public function store(StoreControlSesionRequest $request)
    {
        ...
        // Admin-only rule: do NOT allow two simultaneous Admin sessions on the same totem (receptor)
        // External remains permissive and does not block by receptor.
        $isAdminPanel = request()->routeIs('admin.*');
        if ($isAdminPanel && $receptorId) {
            // Look for any active session on this receptor whose emisor is an Admin (non-external) user
            ...
            foreach ($activeOnReceptor as $sess) {
                ...
                $emisor = $sess->emisor;
                // If we cannot determine, be conservative and treat as Admin to avoid duplicates in Admin
                $isExternalEmisor = ($emisor && method_exists($emisor, 'isExternal')) ? (bool)$emisor->isExternal() : false;
                if (!$isExternalEmisor) { $hasAdminOwner = true; break; }
            }
            if ($hasAdminOwner) {
                return back()->with(['message' => 'Este tótem ya tiene una sesión de control activa en Admin.']);
            }
        }
    ...
    public function ShowEventManager(ControlSesion $controlSesion){
        ...
        if(isset($control_actual->id)){
            ...
            return view('admin.manager.control',compact([
            ...
            return view('admin.manager.control',compact(['controlSesion']));
        }
    ...
    public function edit(ControlSesion $controlSesion)
    {
        ...
        return view('admin.controlSesions.edit', compact('controlSesion', 'emisors', 'receptors'));
    }
    ...
    public function update(UpdateControlSesionRequest $request, ControlSesion $controlSesion)
    {
        ...
        return redirect()->route('admin.control-sesions.index');
    }
    ...
    public function show(ControlSesion $controlSesion)
    {
        ...
        return view('admin.controlSesions.show', compact('controlSesion'));
    }
