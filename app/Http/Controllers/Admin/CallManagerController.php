...
class CallManagerController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if (isset($control_actual->id)) {
            ...
            return view('admin.callManager.index', compact([
                'control_actual',
            ...
            return view('admin.callManager.index');
        }
    }
    ...
    public function recommendations(Request $request)
    {
        ...
        try {
            ...
            if (!$resp->successful()) {
                Log::warning('ARI recommendation error', [
                    'url' => $url,
                    'status' => $resp->status(),
                    'body' => $resp->body(),
                    'payload' => $payload,
            ...
            Log::error('ARI recommendation exception', [
                'url' => $url,
                'payload' => $payload,
    ...
    public function reservationRecommendations(Request $request)
    {
        ...
        Log::info('ARI reservationRecommendations requested', [
            'external_id' => (string) $establecimientoId,
        ...
        Log::info('ARI reservationRecommendations outbound', [
            'url' => $url,
            'base_url' => $baseUrl,
            'payload' => $payload,
        ]);
        ...
        try {
            ...
            Log::info('ARI reservationRecommendations response', [
                'status' => $resp->status(),
            ]);

            // Guardar también un fragmento del body para confirmar que viene de ARI (sin llenar logs)
            try {
                Log::debug('ARI reservationRecommendations response body (truncated)', [
                    'status' => $resp->status(),
            ...
            if (!$resp->successful()) {
                Log::warning('ARI reservation recommendation error', [
                    'url' => $url,
                    'status' => $resp->status(),
                    'body' => $resp->body(),
                    'payload' => $payload,
            ...
            Log::error('ARI reservation recommendation exception', [
                'url' => $url,
                'payload' => $payload,
                'exception' => $e->getMessage(),
            ]);
    ...
    public function paymentTable(Request $request, $folio_id)
    {
        ...
        return view('admin.callManager.paymentTable', compact('pagos_sesion'));
    }
    ...
    public function writeCardTable(Request $request)
    {
        ...
        return view('admin.callManager.writeCardTable', compact('tarjetas_sesion'));
    }
