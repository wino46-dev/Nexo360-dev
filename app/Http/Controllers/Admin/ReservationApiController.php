...
class ReservationApiController extends Controller
{
    ...
    public function folioDetail(Request $request)
    {
        ...
        if (!$id) {
            ...
            return view('admin.callManager.reservation.folioDetail', compact('folio'));
        }
    }
    ...
    public function folioDetailSelect(Request $request)
    {
        ...
        return view('admin.callManager.reservation.folioDetail', ['folio' => $folio, 'origen' => 'folio_selected']);
    }
    ...
    public function reservationSelectOld(Request $request, $id)
    {
        ...
        return view(
            'admin.callManager.reservation.checkin',
            compact(
                'reservation',
                'checkinPartners',
                'documentTypesList',
    ...
    public function checkinPartner(Request $request)
    {
        ...
        if (isset($data['action']) && $data['action'] == 'checkin_save') {
            ...
            if (!empty($checkin_partner['error'])) {
                ...
                // Forzar recálculo económico en PMS
                try {
                    \Log::info('[checkin_partial][recalc][admin] start', ['reservation_id' => (int)$reservation_id]);
                    $hotel_api->reservationDetail((int)$reservation_id);
                } catch (\Throwable $e) {
                    \Log::warning('[checkin_partial][recalc][admin] fail', ['reservation_id' => (int)$reservation_id, 'err' => $e->getMessage()]);
                }
    ...
    public function folioStatus(Request $request)
    {
        ...
        return view(
            'admin.callManager.reservation._status_reservations',
            compact('reservations_checkin_status', 'reservations_checkout_status', 'error_message')
        );
    }

    /**
     * Devuelve el HTML con el desglose de costes de una reserva (solo lectura)
     * Contexto: panel admin/call manager
     */
    public function reservationCosts(Request $request, $id)
    {
        ...
        try {
            ...
            \Log::info('[reservationCosts][ADMIN] load', [
                'sid' => $sid,
                'reservation_id' => (int)$id,
            ...
            \Log::warning('[reservationCosts][ADMIN] log error: '.$e->getMessage());
        }

        if (isset($reservation['error'])) {
            \Log::error('[reservationCosts][ADMIN] API error', ['reservation_id' => (int)$id, 'error' => $reservation['error'] ?? null]);
            return response()->json(['error' => $reservation['error']], 500);
        }

        // Reutilizamos la misma vista parcial
