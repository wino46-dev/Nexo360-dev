...
class CheckInController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('admin.checkIns.index', compact('reservas', 'habitacions', 'totems', 'teams', 'defaultEstablecimientoId', 'defaultEstablecimientoCodigo', 'hasActiveControlSesion'));
    }
    ...
    public function create()
    {
        ...
        return view('admin.checkIns.create', compact('clientes', 'habitacions', 'reservas', 'totems'));
    }
    ...
    public function edit(CheckIn $checkIn)
    {
        ...
        return view('admin.checkIns.edit', compact('checkIn', 'clientes', 'habitacions', 'reservas', 'totems'));
    }
    ...
    public function parteViajeroPdf(Request $request)
    {
        ...
        if (File::exists($file_origen)) {
            ...
            return view('admin.checkIns._parte_viajero_pdf', compact('url_pdf'));
        }
    }
    ...
    {
        abort_if(Gate::denies('check_in_edit') && Gate::denies('check_in_access'), Response::HTTP_FORBIDDEN, '403 Forbidden');
        $ids = (array)($request->input('ids', []));
        $emailsInput = trim((string)$request->input('email', ''));
        if (empty($ids)) {
            return response()->json(['status' => 'error', 'message' => 'No hay check-ins seleccionados'], 422);
        }
        $checkIns = CheckIn::whereIn('id', $ids)->get();
        if ($checkIns->isEmpty()) {
            return response()->json(['status' => 'error', 'message' => 'No se encontraron check-ins'], 404);
        }
        // Determinar reserva y establecimiento a partir del primer check-in
        $first = $checkIns->first();
        $reservation = Reservation::find($first->reservation_id);
        if (!$reservation) {
            return response()->json(['status' => 'error', 'message' => 'Reserva no encontrada para el check-in seleccionado'], 404);
        }
        // Intentar resolver establecimiento para usar en el mail
        $establecimiento_datos = null;
        try {
            $folio = \App\Models\Folio::find($reservation->folio_id);
            if ($folio) {
                $establecimiento_datos = \App\Models\Establecimiento::find($folio->establecimiento_id);
            }
        } catch (\Throwable $e) {
            // opcional
        }
        // Generar PDFs si no existen aún (usamos PMS local para asegurar)
        try {
            \App\Services\ParteViajeroService::pdfGenerate($reservation->id, null, 'local');
        } catch (\Throwable $e) {
            // Seguiremos igualmente; los adjuntos que existan se añadirán en el Mailable
        }
        // Destinatarios
        $emails = [];
        if ($emailsInput !== '') {
            $emails = array_values(array_filter(array_map('trim', explode(',', $emailsInput))));
        } else {
            // Recoger emails de los propios check-ins
            $emails = $checkIns->pluck('email')->filter()->unique()->values()->all();
        }
        if (empty($emails)) {
            return response()->json(['status' => 'error', 'message' => 'No hay email de destino. Indique uno o complete el email en los check-ins.'], 422);
        }
        try {
            \Mail::to($emails)->send(new \App\Mail\ParteViajeroMail($reservation, $establecimiento_datos, $checkIns, true, false));
            return response()->json(['status' => 'ok', 'sent_to' => $emails], 200);
        } catch (\Throwable $e) {
            \Log::error('Error al enviar parte viajero desde listado', ['error' => $e->getMessage()]);
            return response()->json(['status' => 'error', 'message' => 'Error interno al enviar el email'], 500);
        }
    }
}
