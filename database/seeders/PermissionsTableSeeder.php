    ...
    public function run()
    {
        $permissions = [
            ...
            'profile_password_edit',
                        ...
                        // FTP Upload Logs permissions
                        'ftp_upload_log_access',
                        'ftp_upload_log_show',
        ...
        // Create "FTP Auditor" role with read-only access to FTP logs
        $ftpRoleId = DB::table('roles')->where('title', 'FTP Auditor')->value('id');
        if (!$ftpRoleId) {
            $ftpRoleId = DB::table('roles')->insertGetId(['title' => 'FTP Auditor']);
        }
        // Ensure required permissions exist
        $needPerms = DB::table('permissions')->whereIn('title', [
            'auditorium_access', 'ftp_upload_log_access', 'ftp_upload_log_show'
        ])->pluck('id', 'title');
        foreach (['auditorium_access','ftp_upload_log_access','ftp_upload_log_show'] as $pTitle) {
            $permId = $needPerms[$pTitle] ?? null;
            if ($permId) {
                $exists = DB::table('permission_role')
                    ->where('role_id', $ftpRoleId)
